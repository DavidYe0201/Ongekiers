...
class Leaderboard extends Component {
  ...
  apiCall = () => {
    ...
    fetch(`https://kamai.tachi.ac/api/v1/games/ongeki/Single/leaderboard`, {
      headers,
    })
  ...
  generateGrid = () => {
    let newJson = [];
    for (let i = 0; i < 100; i++) {
      let username = this.getUsernameFromUserID(
        this.state.leaderboardData.body.gameStats[i].userID
      );
      newJson.push([
        i + 1,
        username,
        this.state.leaderboardData.body.gameStats[
          i
        ].ratings.naiveRating.toFixed(4),
      ]);
    }
    return newJson;
  };
            ...
            {table?.map((data, i) => (
              <TableRow onClick={() => this.function(data)} key={i}>
                {Object.entries(data).map(([key, val], i) => (
                  <TableCell style={styles} key={key}>
                    {val}
                  </TableCell>
                ))}
              </TableRow>
